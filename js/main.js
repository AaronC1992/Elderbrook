...
(function () {
  ...
  function finishIntro() {
    ...
    if (typeof World === 'undefined' || !World || typeof World.navigate !== 'function') {
      if (introFinishRetryCount < 30) {
        introFinishRetryCount++;
        setTimeout(finishIntro, 100);
      } else {
        console.error('Unable to enter game world: World module is not ready.');
      }
      return;
    }
    introFinishRetryCount = 0;
  ...
  document.addEventListener("click", function (e) {
    ...
    switch (action) {
      ...
      case "save-slot":
        ...
        var saveOk = Save.save(slotNum);
        if (saveOk) {
          UI.showMessage('Saved to slot ' + slotNum + '!');
          Audio.play('buttonClick');
        } else {
          UI.showMessage('Save failed. Please try again.');
        }
        UI.renderSaveSlots('save');
        break;
      case "load-slot":
        ...
        break;
      case "delete-slot":
        var deleted = Save.deleteSave(parseInt(btn.getAttribute('data-slot'), 10));
        UI.showMessage(deleted ? 'Save deleted.' : 'Delete failed.');
